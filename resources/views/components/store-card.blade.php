...
<div class="col-lg-4 col-md-6">
    <div class="card h-100 store-card rounded-4" data-store-id="{{ $storeData['id'] }}">
        ...
        <div class="card-img-top position-relative overflow-hidden" style="height: 200px; border-radius: 1rem 1rem 0 0;">
            ...
            <div class="position-absolute bottom-0 start-0 p-3">
                <span class="badge bg-primary px-3 py-2 rounded-pill">
                    <i class="bi {{ $storeData['badge_icon'] }} me-1"></i>
                    {{ $storeData['badge'] }}
                </span>
            </div>
        </div>
        ...
        <div class="card-body p-4">
            <h5 class="card-title fw-bold text-dark mb-3">{{ $storeData['name'] }}</h5>
            <p class="card-text text-muted mb-3">{{ $storeData['description'] }}</p>
            
            <!-- Store Information -->
            <div class="store-info">
                <div class="d-flex align-items-start mb-2">
                    <i class="bi bi-geo-alt-fill text-primary me-3 mt-1"></i>
                    <span class="text-muted">{{ $storeData['address'] }}</span>
                </div>
                <div class="d-flex align-items-center mb-2">
                    <i class="bi bi-telephone-fill text-primary me-3"></i>
                    <a href="tel:{{ $storeData['phone'] }}" class="text-decoration-none text-muted">{{ $storeData['phone'] }}</a>
                </div>
                <div class="d-flex align-items-center mb-3">
                    <i class="bi bi-clock-fill text-primary me-3"></i>
                    <span class="text-muted">{{ $storeData['hours'] }}</span>
                </div>
            </div>
            ...
            <div class="card-buttons">
                <div class="d-grid gap-2">
                    <a href="{{ route('stores.show', $storeData['id']) }}" 
                       class="btn btn-primary rounded-3">
                        <i class="bi bi-info-circle me-2"></i>View Details
                    </a>
                    <a href="{{ $storeData['maps_url'] }}" 
                       target="_blank" 
